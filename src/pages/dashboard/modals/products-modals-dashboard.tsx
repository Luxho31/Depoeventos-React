    ...
    const getProductByIdForm = async (id: number) => {
        try {
            ...
            setSelectedDisciplineIds(
                product.coursesWithSchedules.map(
            ...
            for (let discipline of product.coursesWithSchedules) {
                ...
                form.setFieldsValue({
                    [`days_${discipline.courseId}`]:
                        discipline.schedules[0].days,
                    [`startHour_${discipline.courseId}`]:
                        discipline.schedules[0].startHour,
                    [`endHour_${discipline.courseId}`]:
                        discipline.schedules[0].endHour,
                });
            }
            ...
            form.setFieldsValue({
                ...
                price: product.price,
                categoryId: product.categories.map(
    ...
    const updateProductForm = async (values: any) => {
        try {
            ...
            values.endDate = new Date(values.endDate)
                .toISOString()
                .split("T")[0];
            const coursesWithSchedules = selectedDisciplineIds.map(
                (discipline) => {
                    return {
                        courseId: discipline,
                        schedules: [
                        ...
                        rules={[
                            {
                                required: true,
                                message:
                                    "Por favor ingrese la locación del Producto.",
                            },
                        ]}
